...
.dashboard-workspace{display:grid;grid-template-columns:minmax(0,1fr) 290px;gap:18px;align-items:start}
.dashboard-main-column{display:grid;gap:18px;min-width:0}
.dashboard-workspace .dashboard-lower{grid-template-columns:repeat(2,minmax(0,1fr))}
.dashboard-workspace .quick-panel{position:sticky;top:96px;border-top:4px solid var(--blue);box-shadow:0 16px 34px rgba(15,35,73,.13)}
.dashboard-workspace .quick-panel .quick-list{display:grid;gap:10px}
.dashboard-workspace .quick-panel .quick-list a{display:flex;align-items:center;min-height:46px;padding:11px 13px;border-radius:8px;background:#edf4ff;color:var(--blue);font-weight:800}
.dashboard-workspace .quick-panel .quick-list a:hover{background:var(--blue);color:#fff}
@media(max-width:1100px){.dashboard-workspace{grid-template-columns:1fr}.dashboard-workspace .quick-panel{position:static;grid-row:1}.dashboard-workspace .dashboard-lower{grid-template-columns:1fr}}
...
<section class="dashboard-content">
  ...
  <div class="dashboard-grid dashboard-charts{% if user.role not in ['school_admin','accountant'] %} single{% endif %}">
    ...
    {% if user.role in ['school_admin','accountant'] %}<article class="card fee-card"><div class="panel-heading"><div><h2>Fee Collection</h2><p>Current period</p></div></div><div class="donut-wrap"><div class="donut" style="--value:{{ analytics.get('Fee Collection', 0) if analytics else 0 }}"><div><strong>{{ analytics.get('Fee Collection', 0) if analytics else 0 }}%</strong><span>Collected</span></div></div><ul class="legend"><li><i class="green-dot"></i>Collected</li><li><i class="gold-dot"></i>Pending</li><li><i class="red-dot"></i>Overdue</li></ul></div></article>{% endif %}
  </div>
  ...
  <article class="card quick-panel"><div class="panel-heading"><div><h2>Quick Actions</h2><p>Common tasks</p></div></div><div class="quick-list">{% if user.role == 'system_admin' %}<a href="{{ url_for('schools') }}">＋ Add School</a><a href="{{ url_for('audit_logs') }}">◫ Audit Logs</a>{% elif user.role == 'school_admin' %}<a href="{{ url_for('students') }}">＋ Add Student</a><a href="{{ url_for('teachers') }}">＋ Add Teacher</a><a href="{{ url_for('bulk_import') }}">⇧ Import Students & Teachers</a><a href="{{ url_for('teacher_assignments') }}">▦ Assign Teacher</a><a href="{{ url_for('fees') }}">◈ Record Payment</a><a href="{{ url_for('communications') }}">✉ Send Message</a>{% elif user.role == 'teacher' %}<a href="{{ url_for('attendance') }}">✓ Take Attendance</a><a href="{{ url_for('scores') }}">▤ Record Marks</a><a href="{{ url_for('students') }}">♙ My Students</a>{% elif user.role == 'accountant' %}<a href="{{ url_for('fees') }}">◈ Record Payment</a><a href="{{ url_for('fees') }}">▤ Fee Balances</a>{% elif user.role == 'registrar' %}<a href="{{ url_for('students') }}">＋ Admit Student</a><a href="{{ url_for('calendar') }}">▣ Calendar</a>{% elif user.role == 'receptionist' %}<a href="{{ url_for('students') }}">＋ New Admission</a><a href="{{ url_for('calendar') }}">▣ Calendar</a>{% elif user.role == 'librarian' %}<a href="{{ url_for('library') }}">＋ Add Resource</a><a href="{{ url_for('library') }}">▤ Catalogue</a>{% endif %}</div></article>
  </div>
