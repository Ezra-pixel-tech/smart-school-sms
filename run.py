...
def deliver_communication(item: Communication) -> str:
    ...
    try:
        if item.channel == "email" and os.getenv("SMTP_HOST"):
            ...
            return "sent"
        if item.channel == "sms" and os.getenv("SMS_API_URL"):
            from urllib.parse import urlencode
            from urllib.request import urlopen

            payload = urlencode({"to": item.recipient, "message": item.message}).encode()
            urlopen(os.getenv("SMS_API_URL"), data=payload, timeout=12).read()
            return "sent"
...
{% if user.role == 'school_admin' %}<a href="{{ url_for('students') }}">Students</a><a href="{{ url_for('teachers') }}">Teachers</a><a href="{{ url_for('classes_subjects') }}">Classes & Subjects</a>{% endif %}
